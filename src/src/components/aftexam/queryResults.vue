<template>
    <div class="query-R">
        <!-- <mt-header fixed title="成绩查询" class="getHeight">
        ...
        <div v-show="showFlag">
            <div class="media-wall" v-if="medal_cuss" style="position:fixed;top:.8rem;left:0;right:0">
                ...
                <div v-tap="{methods:gotoMdal,parmers:userId}">
                    <div class="media-bg">
                        <div class="inner">
        ...
        <pulldown>
            <div ref='pullDown' slot='content' class="query-pulldown">
                <div v-show="showFlag" class="show-top">
                    ...
                    <div v-else class="no-result">
                        <p class="f-fixed stu-info" style="background:#fff;padding:.1rem;font-size:.25rem;">
                            <span class="lf">姓名：<i class="color-blue">{{loca_user_name}}</i></span>
                            <span class="rf">省份：<i class="color-blue">{{loca_provence}}</i></span>
                        </p>
                        <p style="margin-top:.2rem;background:#fff;padding:.2rem 0;font-size:.25rem;">
                            暂无数据
                        </p>
                    </div>
                </div>
    ...
    methods: {
        ...
        loadMedalData(q) {
            ...
            if (!q.datas.obj) {
                ...
                return;
            }else if(!q.datas.obj.credentialsList){
                // "我是没有勋章墙"
                $("._v-container").removeClass("ctrmedal");
                return;
            }else{
                $("._v-container").addClass("ctrmedal");
                $("._v-container").css("top","3.3rem");
        ...
        gotoMdal(pa){
            // console.log(pa.parmers);
            // var accontCheck=`{"url":"http://tpl.51bm.net.cn/#/medal/${this.userId}","xinXiTX":"http://tpl.51bm.net.cn/#/Inforperfect"}`;
            // yksapp.goPageWithParams("xzq",accontCheck);
            yksapp.goPageWithParams("xzq",yksapp_user.getUserId());
        }
    },
    updated(){
        // 生成成绩二维码
