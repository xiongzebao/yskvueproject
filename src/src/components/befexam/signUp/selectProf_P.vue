                    ...
                    <ul class="back-prof-list">
                        <li v-for="(asd,idx) in alreadyChoiceProf" :class="{'check-list':asd.checkState}" @click="choice_prof(asd,idx)">
                            ...
                            <div v-else class="button-box">
                                ...
                                <button size="small" class="custom-btn" v-tap.stop="{methods:deletePRof,asd:asd,idx:idx}">    删除该专业
                                </button>
                            </div>
                        </li>
                    </ul>
    ...
    methods: {
        ...
        */
        addProf(){
            ...
            this.$router.push({path:"/examinationPointcollege/testCenterProf",query:this.schInfo});
        },
        // 加载已选专业
        load_prof(datas){
