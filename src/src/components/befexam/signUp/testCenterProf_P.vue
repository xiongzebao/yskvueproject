<template> 
    ...
    <div class="test-selected-prof">
        <mt-header fixed title="选择考点专业">
            <!-- <router-link to="/examinationPointcollege" slot="left">
...
export default {
    ...
    mounted() {
        ...
        // alert(this.$route.query.xueXiaoID)
        ...
        Indicator.open('加载中...');
        var kaodianBM;// 保存在localStorage的考点别名
        ajaxMethod.doAjax(schLIstUrl,schLIstdata,function(datas){
            ...
            if(datas.datas.list.length == 1){
                ...
                // 从考点进来时的特殊处理，从院校进来没有这种特殊需求
                kaodianBM=localStorage.getItem("kaoDianBM");
                // that.kaoShiKDID="asd,asd,"+kaodianBM;
                datas.datas.list[0].examSiteList.forEach(function(value,index){
                    if(value.kaoDianBM==kaodianBM){
                        // alert(kaodianBM)
                        that.kaoShiKDID=value.kaoShiKDID+","+value.kaoDianID+","+value.siteName+","+value.xianKaoZYS;
                    }
                })
                console.log(that.ads_1)
    ...
    methods: {
    	asd(){
            var BMkaoDianID=window.localStorage.getItem("BMkaoDianID");
            this.$router.push({path:"/examinationPointcollege",query:{kaoDianID:BMkaoDianID}});
    	},
        choiceProf_a(){
            var that=this;
            this.ads=[];
                ...
                MessageBox({
                    ...
                    if(action=='confirm'){
                        ...
                        ajaxMethod.doAjax(subUrl,data,function(datas){
                            ...
                            // 传媒类检测是否完善了信息
                            if(!datas.success && datas.code=="student-811"){
                                MessageBox({
                                    title: '提示',
                                    message: datas.message,
                                    showCancelButton: true,
                                ...
                                return;
                            // 上海附中模式检测
                            }else if(!datas.success && datas.code=="student-808"){
                                MessageBox({
                                    title: '提示',
                                    message: datas.message,
                                    showCancelButton: true,
                                    ...
                                    if(action=='confirm'){
                                        that.$router.push({path:"/shAttached"});
                                    }
                                });
                                return;
                                // 西安美术学院户口类型完善
                            }else if(!datas.success && datas.code=="student-812"){
                                MessageBox({
                                    title: '提示',
                                    message: datas.message,
                                    showCancelButton: true,
                                ...
                                that.$router.push({path:"/examinationPointcollege/selectProf",query:{xueXiaoID:that.xueXiaoID,xueXiaoMC:that.selectedSch}})
                                // 旧模式
                                ...
                                // });
                            }else if(datas.code=="424" || datas.code=="495"){
                                MessageBox({
                                    title: '',
                                    message:"此类专业报考需要上传审核资料，请用电脑登录艺考升网站www.artstudent.cn填报",
                                    showCancelButton: false,
                                    confirmButtonText:"知道了",
                                })
                            }else if(datas.code=="306"){
                                MessageBox({
                                    title: '',
                                    message:datas.message,
                                    showCancelButton: false,
                                    ...
                                    var parStr;
                                    if(protocol=="http:"){
                                        parStr=protocol+"//tpl.51bm.net.cn/#/Inforperfect";
                                    }else if(protocol=="https:"){
                                        parStr=protocol+"//tpl.artstudent.cn/#/Inforperfect";
                                    }
                                    yksapp.goPageWithParams('rzIndex',parStr);
                                })
                            }else{
                                MessageBox({
                                    title: '',
