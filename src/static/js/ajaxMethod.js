...
<<<<<<< .mine
    getHlqUrl:function(){//别删掉了，呼啦圈用，呼啦圈和报考共用这个文件
    	return this.protocol+"tpl"+this.domain+"/hlq";
    },
    iosAjax:function(aUrl,data,callback,completeFun,datatype,requestType){
||||||| .r2815
    iosAjax:function(aUrl,data,callback,completeFun,datatype,requestType){
=======
...
>>>>>>> .r3422
		...
		// 演示环境
		// this.url=this.protocol+"192.168.18.200"+this.path;
	    // 手机端获取ticket和uuid
	...
	doAjax:function(aUrl,data,callback,completeFun,datatype,requestType){
		 var that=this;
         if(this.isPc()){
             this.iosAjax(aUrl,data,callback,completeFun,datatype,requestType);
         }else{
            var timer=setInterval(function(){
                  if((window.yksapp)&&(window.yksapp_user)){
                         /* 清除定时器*/
                      timer && clearInterval(timer);
                      that.iosAjax(aUrl,data,callback,completeFun,datatype,requestType);
                  }
             },100)
         }
